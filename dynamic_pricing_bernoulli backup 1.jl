...
md""" # Exemplo Exponencial"""

# ╔═╡ aaf3b36d-4f05-4c72-bcb3-9ea512a69947
ν = Exponential(20)
...
begin
	...
	ylabel!("P(S≤x)")
	# if save_figures
	# 	#savefig("/home/ismael/Documents/Disserta-o/imagens_experimentos_numericos/precificaca_dinamica/exp.png")
	# end
end
...
begin
	...
	# if save_figures
	# 	savefig("/home/ismael/Documents/Disserta-o/imagens_experimentos_numericos/precificaca_dinamica/exp_exemplo.png")
	# end
end
...
# ╔═╡ c3e62eb9-642b-4600-bd28-38b7eefa6399
float(collect(values(a_1))[1])

# ╔═╡ 798ce1fe-09e3-49c7-b403-10bbf02b379c
simulate_pure_exp(arms, H1, "seq_elim", ν, 1)

# ╔═╡ 3bb8d070-f645-4cc8-afd4-cc7c2f11d784
H1
...
a_2, avg_reward_vector_elim = seq_elim(arms, H1, ν)
...
begin
	plot(final_avg_regret_eps[2000:end], label="ε-greedy", dpi=1000)
...
begin
	plot(avg_reward_vector_halving[Int(ceil(0.2*H1)):end], label="rspm", dpi=1000)
	plot!(avg_reward_vector_elim[Int(ceil(0.2*H1)):end], label="")
end

# ╔═╡ ffe321ae-82cf-4128-b03b-7caf7997c358
Int(ceil(0.2*H1))
...
function upper_bound_utc(arms, m, horizon)
	...
	R = m * sum(best_arm_mean .- arms_mean) + (horizon - m*k)*sum((best_arm_mean .- arms_mean) .* exp.(-(m*  (best_arm_mean .- arms_mean).^2)./4 ) ) 
	return R
end

# ╔═╡ 3b7869a3-0898-47a0-afd2-56bd716b8044
upper_bound_utc(arms_wp, 100, 100000)

# ╔═╡ f7195a2f-7434-4247-9dd1-f6a02196964e
md""" ## UCB"""

# ╔═╡ 628fd7a5-fbcb-4175-8a40-2b1322994986
function upper_bound_ucb(arms, n)
	k = length(arms)
	arms_mean = collect(values(arms))
	best_arm_pos = findmax(arms_mean)[2]
	best_arm_mean = findmax(arms_mean)[1]
	Δ = best_arm_mean .- arms_mean
	R = 3 * sum(Δ) + sum(16*log(n) ./ filter(!iszero, Δ))
end

# ╔═╡ f317b1ce-0b18-47bb-86d7-16ff4bc2ac6a
upper_bound_ucb(arms, 10000)
